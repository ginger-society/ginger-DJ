...
from gingerdj.contrib.admin.utils import NotRelationField, flatten, get_fields_from_path
...
from gingerdj.db.models.expressions import Combinable
from gingerdj.forms.models import BaseModelForm, BaseModelFormSet, _get_foreign_key
...
def check_admin_app(app_configs, **kwargs):
    from gingerdj.contrib.admin.sites import all_sites
...
def check_dependencies(**kwargs):
    ...
    if not apps.is_installed("gingerdj.contrib.admin"):
        return []
    errors = []
    app_dependencies = (("gingerdj.contrib.messages", 406),)
    ...
    for engine in engines.all():
        if isinstance(engine, GingerTemplates):
            ginger_templates_instance = engine.engine
            break
    else:
        ginger_templates_instance = None
    if not ginger_templates_instance:
        errors.append(
            checks.Error(
                "A 'gingerdj.template.backends.gingerdj.GingerTemplates' instance "
        ...
        if (
            ...
            not in ginger_templates_instance.context_processors
        ):
            errors.append(
                checks.Error(
        ...
        if (
            sidebar_enabled
            and "gingerdj.template.context_processors.request"
            not in ginger_templates_instance.context_processors
        ):
            errors.append(
                checks.Warning(
    ...
    if not _contains_subclass(
        "gingerdj.contrib.messages.middleware.MessageMiddleware", settings.MIDDLEWARE
    ):
        errors.append(
            checks.Error(
                "'gingerdj.contrib.messages.middleware.MessageMiddleware' must "
...
class BaseModelAdminChecks:
    ...
    def _check_radio_fields_value(self, obj, val, label):
        ...
        from gingerdj.contrib.admin.options import HORIZONTAL, VERTICAL
...
class ModelAdminChecks(BaseModelAdminChecks):
    ...
    def _check_inlines_item(self, obj, inline, label):
        ...
        from gingerdj.contrib.admin.options import InlineModelAdmin
    ...
    def _check_list_filter_item(self, obj, item, label):
        ...
        """
        from gingerdj.contrib.admin import FieldListFilter, ListFilter

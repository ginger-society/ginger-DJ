...
def SetupPrometheusEndpointOnPort(port, addr=""):
    ...
    all interfaces. This HTTPServer is fully independent of GingerDJ and
    its stack. This offers the advantage that even if GingerDJ becomes
    unable to respond, the HTTPServer will continue to function and
    export metrics. However, this also means that the features
    offered by GingerDJ (like middlewares or WSGI) can't be used.

    Now here's the really weird part. When GingerDJ runs with the
    ...
    assert os.environ.get("RUN_MAIN") != "true", (
        "The thread-based exporter can't be safely used when gingerdj's "
        "autoreloader is active. Use the URL exporter, or start gingerdj "
...
def SetupPrometheusEndpointOnPortRange(port_range, addr=""):
    ...
    This is useful when you're running GingerDJ as a WSGI application
    ...
    GingerDJ's autoreloader is active.
    """
    assert os.environ.get("RUN_MAIN") != "true", (
        "The thread-based exporter can't be safely used when gingerdj's "
        "autoreloader is active. Use the URL exporter, or start gingerdj "
...
def ExportToGingerView(request):
    """Exports /metrics as a GingerDJ view.

    You can use prometheus.urls to map /metrics to this view.
    """

...
function patchSwaggerUi() {
    if (document.querySelector(".auth-wrapper #gingerdj-session-auth")) {
        return;
    }

    var authWrapper = document.querySelector(".auth-wrapper");
    var authorizeButton = document.querySelector(".auth-wrapper .authorize");
    var djangoSessionAuth = document.querySelector("#gingerdj-session-auth");
...
function initSwaggerUi() {
    if (window.ui) {
        console.log(
            "WARNING: skipping initSwaggerUi() because window.ui is already defined"
        );
        return;
    }
    ...
    var swaggerSettings = JSON.parse(
        document.getElementById("swagger-settings").innerHTML
    );
    ...
    var oauth2Config = JSON.parse(
        document.getElementById("oauth2-config").innerHTML
    );
    ...
    if (persistAuth) {
        try {
            savedAuth = Immutable.fromJS(
                JSON.parse(localStorage.getItem(KEY_AUTH)) || {}
            );
        } catch (e) {
            ...
            if (key && paramName) {
                ...
                if (_in === "query") {
                    if (requestUrl) {
                        ...
                        console.warn(
                            "WARNING: cannot apply apiKey query parameter via interceptor"
                        );
                    }
                }
            }

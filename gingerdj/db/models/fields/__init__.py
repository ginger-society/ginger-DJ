...
from gingerdj.core import checks, exceptions, validators
from gingerdj.db import connection, connections, router
from gingerdj.db.models.constants import LOOKUP_SEP
from gingerdj.db.models.query_utils import DeferredAttribute, RegisterLookupMixin
from gingerdj.db.utils import NotSupportedError
...
from gingerdj.utils.datastructures import DictWrapper
...
from gingerdj.utils.duration import duration_microseconds, duration_string
from gingerdj.utils.functional import Promise, cached_property
from gingerdj.utils.ipv6 import clean_ipv6_address
from gingerdj.utils.text import capfirst
...
class Field(RegisterLookupMixin):
    ...
    # The auto_creation_counter is used for fields that GingerDJ implicitly
    ...
    def deconstruct(self):
         ...
         * The import path of the field, including the class, e.g.
           gingerdj.db.models.IntegerField. This should be the most portable
        ...
        for name, default in possibles.items():
            ...
            if name in equals_comparison:
                ...
                if value is not default:
                    keywords[name] = value
        # Work out path - we shorten it for known GingerDJ core fields
        path = "%s.%s" % (self.__class__.__module__, self.__class__.__qualname__)
        if path.startswith("gingerdj.db.models.fields.related"):
            path = path.replace("gingerdj.db.models.fields.related", "gingerdj.db.models")
        elif path.startswith("gingerdj.db.models.fields.files"):
            path = path.replace("gingerdj.db.models.fields.files", "gingerdj.db.models")
        elif path.startswith("gingerdj.db.models.fields.generated"):
            path = path.replace("gingerdj.db.models.fields.generated", "gingerdj.db.models")
        elif path.startswith("gingerdj.db.models.fields.json"):
            path = path.replace("gingerdj.db.models.fields.json", "gingerdj.db.models")
        elif path.startswith("gingerdj.db.models.fields.proxy"):
            path = path.replace("gingerdj.db.models.fields.proxy", "gingerdj.db.models")
        elif path.startswith("gingerdj.db.models.fields"):
            path = path.replace("gingerdj.db.models.fields", "gingerdj.db.models")
    ...
    def to_python(self, value):
        ...
        gingerdj.core.exceptions.ValidationError if the data can't be converted.
    ...
    def db_type(self, connection):
        ...
        # which *preexisting* GingerDJ Field class it's most similar to -- i.e.,
        # a custom field might be represented by a TEXT column type, which is
        # the same as the TextField GingerDJ field type, which means the custom
        # field's get_internal_type() returns 'TextField'.
        #
        # But the limitation of the get_internal_type() / data_types approach
        # is that it cannot handle database column types that aren't already
        # mapped to one of the built-in GingerDJ field types. In this case, you
    ...
    def db_returning(self):
        """Private API intended only to be used by GingerDJ itself."""
        return (
    ...
    def pre_save(self, model_instance, add):
        ...
        if not connection.features.supports_default_keyword_in_insert:
            from gingerdj.db.models.expressions import DatabaseDefault
    ...
    def formfield(self, form_class=None, choices_form_class=None, **kwargs):
        """Return a gingerdj.forms.Field instance for this field."""
        defaults = {
...
class CharField(Field):
    ...
    def slice_expression(self, expression, start, length):
        from gingerdj.db.models.functions import Substr
...
class AutoFieldMeta(type):
    ...
    In many areas GingerDJ also relies on using isinstance() to check for an

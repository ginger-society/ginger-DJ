...
from gingerdj.utils.jslex import prepare_js_for_gettext
from gingerdj.utils.regex_helper import _lazy_re_compile
from gingerdj.utils.text import get_text_list
from gingerdj.utils.translation import templatize
...
class BuildFile:
    ...
    def is_templatized(self):
        if self.domain == "gingerjs":
            return self.command.gettext_version < (0, 18, 3)
        elif self.domain == "gingerdj":
...
class Command(BaseCommand):
    help = (
        ...
        "conf/locale (in the gingerdj tree) or locale (for projects and applications) "
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            "-d",
            default="gingerdj",
            help='The domain of the message files (default: "gingerdj").',
        )
        parser.add_argument(
            "--all",
            "-a",
        ...
        parser.add_argument(
            ...
            'if the domain is "gingerjs"). Separate multiple extensions with '
    ...
    def handle(self, *args, **options):
        ...
        if self.domain not in ("gingerdj", "gingerjs"):
            raise CommandError(
                "currently makemessages only supports domains "
                "'gingerdj' and 'gingerjs'"
            )
        if self.domain == "gingerjs":
    ...
    def process_locale_dir(self, locale_dir, files):
        ...
        for translatable in files:
            ...
            if self.domain not in ("gingerjs", "gingerdj"):
                continue
    ...
    def write_po_file(self, potfile, locale):
        ...
        if os.path.exists(pofile):
            ...
            with open(potfile, encoding="utf-8") as fp:
                msgs = fp.read()
            if not self.invoked_for_ginger:
    ...
    def copy_plural_forms(self, msgs, locale):
        """
        Copy plural forms header contents from a GingerDJ catalog of locale to
        the msgs string, inserting it at the right place. msgs should be the
        contents of a newly created .po file.
        """
        ginger_dir = os.path.normpath(os.path.join(os.path.dirname(gingerdj.__file__)))
        if self.domain == "gingerjs":
            domains = ("gingerjs", "gingerdj")
        else:
            domains = ("gingerdj",)
        for domain in domains:
            ginger_po = os.path.join(
                ginger_dir, "conf", "locale", locale, "LC_MESSAGES", "%s.po" % domain
            )
            if os.path.exists(ginger_po):
                with open(ginger_po, encoding="utf-8") as fp:

...
from gingerdj.db import connections, router
...
from gingerdj.utils.formats import FORMAT_SETTINGS, reset_format_cache
...
def clear_cache_handlers(*, setting, **kwargs):
    if setting == "CACHES":
        from gingerdj.core.cache import caches, close_caches
...
def update_installed_apps(*, setting, **kwargs):
    if setting == "INSTALLED_APPS":
        ...
        from gingerdj.contrib.staticfiles.finders import get_finder

        get_finder.cache_clear()
        # Rebuild management commands cache
        from gingerdj.core.management import get_commands

        get_commands.cache_clear()
        # Rebuild get_app_template_dirs cache.
        from gingerdj.template.utils import get_app_template_dirs
...
def storages_changed(*, setting, **kwargs):
    ...
    from gingerdj.core.files.storage import default_storage, storages
...
def root_urlconf_changed(*, setting, **kwargs):
    if setting == "ROOT_URLCONF":
        from gingerdj.urls import clear_url_caches, set_urlconf

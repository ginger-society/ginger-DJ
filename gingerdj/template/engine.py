...
class Engine:
    ...
    def __init__(
        ...
        if loaders is None:
            ...
            loaders = [("gingerdj.template.loaders.cached.Loader", loaders)]
        else:
            if app_dirs:
    ...
    def get_default():
        """
        Return the first GingerTemplates backend that's configured, or raise
        ...
        'Hello world!'
        """
        # Since Engine is imported in gingerdj.template and since
        # GingerTemplates is a wrapper around this Engine class,
        ...
        raise ImproperlyConfigured("No GingerTemplates backend is configured.")
    ...
    def render_to_string(self, template_name, context=None):
        ...
        For use in GingerDJ's test suite.
        """
        if isinstance(template_name, (list, tuple)):
            t = self.select_template(template_name)
        else:
            t = self.get_template(template_name)
        # GingerDJ < 1.8 accepted a Context in `context` even though that's

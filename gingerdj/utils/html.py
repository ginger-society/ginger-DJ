...
from gingerdj.utils.encoding import punycode
from gingerdj.utils.functional import Promise, keep_lazy, keep_lazy_text
from gingerdj.utils.http import RFC3986_GENDELIMS, RFC3986_SUBDELIMS
...
from gingerdj.utils.text import normalize_newlines
...
def json_script(value, element_id=None, encoder=None):
    ...
    json_str = json.dumps(value, cls=encoder or GingerJSONEncoder).translate(
        _json_script_escapes
    )
    if element_id:
...
def conditional_escape(text):
    ...
    This function relies on the __html__ convention used both by GingerDJ's
...
def html_safe(klass):
    """
    A decorator that defines the __html__ method. This helps non-GingerDJ

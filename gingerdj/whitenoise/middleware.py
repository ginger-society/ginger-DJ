...
from gingerdj.contrib.staticfiles import finders
...
class WhiteNoiseFileResponse(FileResponse):
    """
    Wrap gingerdj's FileResponse to prevent setting any default headers. For the
    most part these just duplicate work already done by WhiteNoise but in some
    cases (e.g. the content-disposition header introduced in gingerdj 3.0) they
...
class WhiteNoiseMiddleware(WhiteNoise):
    """
    Wrap WhiteNoise to allow it to function as gingerdj middleware, rather
    ...
    def get_name_without_hash(self, filename):
        ...
        Note: this is specific to the naming scheme used by gingerdj's

...
from ginger.core import checks, exceptions, validators
from ginger.db import connection, connections, router
from ginger.db.models.constants import LOOKUP_SEP
from ginger.db.models.query_utils import DeferredAttribute, RegisterLookupMixin
from ginger.db.utils import NotSupportedError
...
from ginger.utils.datastructures import DictWrapper
...
from ginger.utils.duration import duration_microseconds, duration_string
from ginger.utils.functional import Promise, cached_property
from ginger.utils.ipv6 import clean_ipv6_address
...
class Field(RegisterLookupMixin):
    ...
    # The auto_creation_counter is used for fields that Ginger implicitly
    ...
    def deconstruct(self):
         ...
         * The import path of the field, including the class, e.g.
           ginger.db.models.IntegerField. This should be the most portable
        ...
        for name, default in possibles.items():
            ...
            if name in equals_comparison:
                ...
                if value is not default:
                    keywords[name] = value
        # Work out path - we shorten it for known Ginger core fields
        path = "%s.%s" % (self.__class__.__module__, self.__class__.__qualname__)
        if path.startswith("ginger.db.models.fields.related"):
            path = path.replace("ginger.db.models.fields.related", "ginger.db.models")
        elif path.startswith("ginger.db.models.fields.files"):
            path = path.replace("ginger.db.models.fields.files", "ginger.db.models")
        elif path.startswith("ginger.db.models.fields.generated"):
            path = path.replace("ginger.db.models.fields.generated", "ginger.db.models")
        elif path.startswith("ginger.db.models.fields.json"):
            path = path.replace("ginger.db.models.fields.json", "ginger.db.models")
        elif path.startswith("ginger.db.models.fields.proxy"):
            path = path.replace("ginger.db.models.fields.proxy", "ginger.db.models")
        elif path.startswith("ginger.db.models.fields"):
            path = path.replace("ginger.db.models.fields", "ginger.db.models")
    ...
    def to_python(self, value):
        ...
        ginger.core.exceptions.ValidationError if the data can't be converted.
    ...
    def db_type(self, connection):
        ...
        # which *preexisting* Ginger Field class it's most similar to -- i.e.,
        # a custom field might be represented by a TEXT column type, which is
        # the same as the TextField Ginger field type, which means the custom
        # field's get_internal_type() returns 'TextField'.
        #
        # But the limitation of the get_internal_type() / data_types approach
        # is that it cannot handle database column types that aren't already
        # mapped to one of the built-in Ginger field types. In this case, you
    ...
    def db_returning(self):
        """Private API intended only to be used by Ginger itself."""
        return (
    ...
    def pre_save(self, model_instance, add):
        ...
        if not connection.features.supports_default_keyword_in_insert:
            from ginger.db.models.expressions import DatabaseDefault
    ...
    def formfield(self, form_class=None, choices_form_class=None, **kwargs):
        """Return a ginger.forms.Field instance for this field."""
        defaults = {
...
class CharField(Field):
    ...
    def slice_expression(self, expression, start, length):
        from ginger.db.models.functions import Substr
...
class AutoFieldMeta(type):
    ...
    In many areas Ginger also relies on using isinstance() to check for an

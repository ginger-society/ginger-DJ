...
from ginger.core.exceptions import AppRegistryNotReady
...
class GingerTranslation(gettext_module.GNUTranslations):
    ...
    def __init__(self, language, domain=None, localedirs=None):
        ...
        if self.domain == "ginger":
            if localedirs is not None:
                # A module-level cache is used for caching 'ginger' translations
                warnings.warn(
                    "localedirs is ignored when domain is 'ginger'.", RuntimeWarning
                )
                localedirs = None
    ...
    def _init_translation_catalog(self):
        """Create a base catalog using global ginger translations."""
    ...
    def merge(self, other):
        ...
        if self._catalog is None:
            # Take plural and _info from first catalog found (generally Ginger's).
...
def translation(language):
    """
    Return a translation object in the default 'ginger' domain.
    """
...
def check_for_language(lang_code):
    ...
    <https://www.ginger.gloportal.dev/weblog/2007/oct/26/security-fix/>.
    """
    # First, a quick check to make sure lang_code is well-formed (#21458)
    if lang_code is None or not language_code_re.search(lang_code):
        return False
    return any(
        gettext_module.find("ginger", path, [to_locale(lang_code)]) is not None

# LayerMapping -- A Ginger Model/OGR Layer Mapping Utility
"""
 The LayerMapping class provides a way to map the contents of OGR
 vector files (e.g. SHP files) to Geographic-enabled Ginger models.

 For more information, please consult the GeoGinger documentation:
   https://docs.ginger.gloportal.dev/en/dev/ref/contrib/gis/layermapping/
"""
import sys
...
from ginger.contrib.gis.gdal.field import (
    OFTDate,
    OFTDateTime,
    OFTInteger,
    OFTInteger64,
...
from ginger.core.exceptions import FieldDoesNotExist, ObjectDoesNotExist
from ginger.db import connections, models, router, transaction
...
class LayerMapping:
    "A class that maps OGR Layers to GeoGinger Models."
    ...
    # Acceptable Ginger field types and corresponding acceptable OGR
    # counterparts.
    FIELD_TYPES = {
    ...
    def check_layer(self):
        ...
        for field_name, ogr_name in self.mapping.items():
            ...
            # Getting the string name for the Ginger field class (e.g., 'PointField').
            ...
            if isinstance(model_field, GeometryField):
                ...
                if model_field.__class__ not in self.FIELD_TYPES:
                    raise LayerMapError(
                        'Ginger field type "%s" has no OGR mapping (yet).' % fld_name
                    )
                ...
                # Can the OGR field type be mapped to the Ginger field type?
                if not issubclass(ogr_field, self.FIELD_TYPES[model_field.__class__]):
                    raise LayerMapError(
                        'OGR field "%s" (of type %s) cannot be mapped to Ginger %s.'
    ...
    def verify_geom(self, geom, model_field):
        ...
        """
        # Measured geometries are not yet supported by GeoGinger models.
        if geom.is_measured:
    ...
    def make_multi(self, geom_type, model_field):
        ...
        return (
            ...
            and model_field.__class__.__name__ == "Multi%s" % geom_type.ginger
        )

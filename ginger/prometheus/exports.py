...
def SetupPrometheusEndpointOnPort(port, addr=""):
    ...
    all interfaces. This HTTPServer is fully independent of Ginger and
    its stack. This offers the advantage that even if Ginger becomes
    unable to respond, the HTTPServer will continue to function and
    export metrics. However, this also means that the features
    offered by Ginger (like middlewares or WSGI) can't be used.

    Now here's the really weird part. When Ginger runs with the
    ...
    assert os.environ.get("RUN_MAIN") != "true", (
        "The thread-based exporter can't be safely used when ginger's "
        "autoreloader is active. Use the URL exporter, or start ginger "
...
def SetupPrometheusEndpointOnPortRange(port_range, addr=""):
    ...
    This is useful when you're running Ginger as a WSGI application
    ...
    Ginger's autoreloader is active.
    """
    assert os.environ.get("RUN_MAIN") != "true", (
        "The thread-based exporter can't be safely used when ginger's "
        "autoreloader is active. Use the URL exporter, or start ginger "
...
def ExportToGingerView(request):
    """Exports /metrics as a Ginger view.

"""
Sphinx plugins for GingerDJ documentation.
"""
...
def setup(app):
    ...
    app.add_object_type(
        directivename="gingerdj-admin",
        rolename="djadmin",
        indextemplate="pair: %s; gingerdj-admin command",
        parse_node=parse_ginger_admin_node,
    )
    app.add_directive("gingerdj-admin-option", Cmdoption)
    app.add_config_value("ginger_next_version", "0.0", True)
    app.add_directive("versionadded", VersionDirective)
    app.add_directive("versionchanged", VersionDirective)
    app.add_builder(GingerStandaloneHTMLBuilder)
    app.set_translator("gingerhtml", GingerHTMLTranslator)
    app.set_translator("json", GingerHTMLTranslator)
    app.add_node(
        ConsoleNode,
...
class VersionDirective(Directive):
    ...
    def run(self):
        ...
        if self.arguments[0] == env.config.ginger_next_version:
...
class GingerHTMLTranslator(HTMLTranslator):
    """
    GingerDJ-specific reST to HTML tweaks.
    """
    ...
    #
    version_text = {
        "versionchanged": "Changed in GingerDJ %s",
        "versionadded": "New in GingerDJ %s",
    }
...
def parse_ginger_admin_node(env, sig, signode):
    command = sig.split(" ")[0]
    env.ref_context["std:program"] = command
    title = "gingerdj-admin %s" % sig
    signode += addnodes.desc_name(title, title)
    return command


class GingerStandaloneHTMLBuilder(StandaloneHTMLBuilder):
    """
    ...
    def finish(self):
        ...
        with open(outfilename, "w") as fp:
            fp.write("var ginger_template_builtins = ")
...
def visit_console_html(self, node):
    ...
    if self.builder.name in ("gingerhtml", "json") and node["win_console_text"]:
...
class ConsoleDirective(CodeBlock):
    ...
    def run(self):
        ...
        # Only do work when the gingerhtml HTML Sphinx builder is being used,
        # invoke the default behavior for the rest.
        if env.app.builder.name not in ("gingerhtml", "json"):
            return [lit_blk_obj]

...
class Tests(TestCase):
    ...
    def test_service_name_default_db(self):
        ...
        settings = connection.settings_dict.copy()
        settings["NAME"] = None
        settings["OPTIONS"] = {"service": "ginger_test"}
        params = DatabaseWrapper(settings).get_connection_params()
    ...
    def test_connect_role(self):
        ...
        """
        try:
            custom_role = "ginger_nonexistent_role"
            new_connection = no_pool_connection()
    ...
    def test_copy_to_expert_cursors(self):
        out = StringIO()
        copy_expert_sql = "COPY ginger_session TO STDOUT (FORMAT CSV, HEADER)"
        with connection.cursor() as cursor:
            cursor.copy_expert(copy_expert_sql, out)
            cursor.copy_to(out, "ginger_session")
        self.assertEqual(
            [q["sql"] for q in connection.queries],
            [copy_expert_sql, "COPY ginger_session TO STDOUT"],
        )

    @override_settings(DEBUG=True)
    @unittest.skipUnless(is_psycopg3, "psycopg3 specific test")
    def test_copy_cursors(self):
        copy_sql = "COPY ginger_session TO STDOUT (FORMAT CSV, HEADER)"
        with connection.cursor() as cursor:

...
class DiscoverRunnerParallelArgumentTests(SimpleTestCase):
    ...
    def test_get_max_test_processes_spawn(
        ...
        self.assertEqual(get_max_test_processes(), 12)
        with mock.patch.dict(os.environ, {"GINGER_TEST_PROCESSES": "7"}):
            self.assertEqual(get_max_test_processes(), 7)
...
class DiscoverRunnerTests(SimpleTestCase):
    ...
    def test_dotted_test_class_ginger_testcase(self):
        count = (
            DiscoverRunner(verbosity=0)
            .build_suite(
    ...
    def test_dotted_test_method_ginger_testcase(self):
        count = (
            DiscoverRunner(verbosity=0)
            .build_suite(
                ["test_runner_apps.sample.tests_sample.TestGingerTestCase.test_sample"],
            )
            .countTestCases()
        )
    ...
    def test_testcase_ordering(self):
        with change_cwd(".."):
            ...
            self.assertEqual(
                ...
                "TestGingerTestCase",
                msg="TestGingerTestCase should be the first test case",
            )
            self.assertEqual(
    ...
    def test_reverse(self):
        ...
        suite = tuple(suite)
        self.assertIn(
            "GingerCase", suite[0].id(), msg="Test groups should not be reversed."
        )
        self.assertIn(
    ...
    def test_number_of_databases_no_parallel_test_suite(self):
        ...
        """
        runner = DiscoverRunner(parallel=8, verbosity=0)
        suite = runner.build_suite(["test_runner_apps.simple.tests.GingerCase1"])

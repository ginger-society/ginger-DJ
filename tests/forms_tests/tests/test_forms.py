...
Password: <input type="password" name="password" aria-invalid="true" required></li>""",
    ...
    def test_callable_initial_data(self):
        ...
        # Here, we're not submitting any data, so the initial value will be displayed.)
        p = UserRegistration(
            initial={"username": initial_ginger, "options": initial_options},
            auto_id=False,
        )
        self.assertHTMLEqual(
            p.as_ul(),
...
<option value="w">whiz</option>
        ...
        p = UserRegistration(
            {"username": ""}, initial={"username": initial_ginger}, auto_id=False
        )
        self.assertHTMLEqual(
            p.as_ul(),
            """<li><ul class="errorlist"><li>This field is required.</li></ul>
...
<option value="w">whiz</option>
        ...
        p = UserRegistration(
            ...
            initial={"username": initial_ginger},
            auto_id=False,
        )
        self.assertHTMLEqual(
            p.as_ul(),
        ...
        # to Form(), then the latter will get precedence.
        class UserRegistration(Form):
            username = CharField(max_length=10, initial=initial_ginger)
            password = CharField(widget=PasswordInput)
            options = MultipleChoiceField(
                choices=[("f", "foo"), ("b", "bar"), ("w", "whiz")],
    ...
    def test_boundfield_values(self):
        ...
        class UserRegistration(Form):
            username = CharField(max_length=10, initial="gingernaut")
            password = CharField(widget=PasswordInput)

        unbound = UserRegistration()
        bound = UserRegistration({"password": "foo"})
        self.assertIsNone(bound["username"].value())
        self.assertEqual(unbound["username"].value(), "gingernaut")
...
class CustomRenderer(GingerTemplates):
...
class RendererTests(SimpleTestCase):
    ...
    def test_attribute_instance(self):
        class CustomForm(Form):
            default_renderer = GingerTemplates()

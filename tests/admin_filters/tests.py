...
class ListFiltersTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        # Books
        cls.gingernaut_book = Book.objects.create(
            title="Gingernaut: an art of living",
            year=2009,
            author=cls.alfred,
            is_best_seller=True,
    ...
    def test_datefieldlistfilter(self):
        ...
        self.assertEqual(list(queryset), [self.ginger_book, self.gingernaut_book])
        ...
        if (self.today.year, self.today.month) == (
            ...
            self.assertEqual(
                list(queryset),
                [self.guitar_book, self.ginger_book, self.gingernaut_book],
            )
        else:
            self.assertEqual(list(queryset), [self.ginger_book, self.gingernaut_book])
        ...
        queryset = changelist.get_queryset(request)
        self.assertEqual(
            list(queryset), [self.guitar_book, self.ginger_book, self.gingernaut_book]
        )
    ...
    def test_allvaluesfieldlistfilter(self):
        ...
        # Make sure the correct queryset is returned
        queryset = changelist.get_queryset(request)
        self.assertEqual(list(queryset), [self.ginger_book])
    ...
    def test_relatedfieldlistfilter_manytomany(self):
        ...
        queryset = changelist.get_queryset(request)
        self.assertEqual(
            list(queryset), [self.ginger_book, self.bio_book, self.gingernaut_book]
        )
    ...
    def test_relatedfieldlistfilter_reverse_relationships(self):
        ...
        request = self.request_factory.get(
            "/", {"books_contributed__id__exact": self.ginger_book.pk}
        )
        request.user = self.alfred
        changelist = modeladmin.get_changelist_instance(request)
        ...
        self.assertEqual(filterspec.title, "book")
        choice = select_by(
            filterspec.choices(changelist), "display", self.ginger_book.title
        )
        self.assertIs(choice["selected"], True)
        self.assertEqual(
            choice["query_string"],
            "?books_contributed__id__exact=%d" % self.ginger_book.pk,
        )

        # With one book, the list filter should appear because there is also a
        # (None) option.
        Book.objects.exclude(pk=self.gingernaut_book.pk).delete()
    ...
    def test_relatedonlyfieldlistfilter_foreignkey_reverse_relationships(self):
        ...
        self.gingernaut_book.employee = self.john
        self.gingernaut_book.save()
        self.ginger_book.employee = self.jack
        self.ginger_book.save()
    ...
    def test_relatedonlyfieldlistfilter_foreignkey_ordering(self):
        ...
        self.gingernaut_book.employee = albert
        self.gingernaut_book.save()
    ...
    def test_listfilter_genericrelation(self):
        ...
        expected = [python_bookmark, ginger_bookmark]
    ...
    def verify_booleanfieldlistfilter(self, modeladmin):
        ...
        # Make sure the correct queryset is returned
        queryset = changelist.get_queryset(request)
        self.assertEqual(list(queryset), [self.guitar_book, self.gingernaut_book])
    ...
    def verify_booleanfieldlistfilter_choices(self, modeladmin):
        ...
        self.assertEqual(
            list(queryset),
            [self.guitar_book, self.ginger_book, self.bio_book, self.gingernaut_book],
        )
        filterspec = changelist.get_filters(request)[0][6]
        self.assertEqual(filterspec.title, "availability")
    ...
    def test_fieldlistfilter_underscorelookup_tuple(self):
        ...
        # Make sure the correct queryset is returned
        queryset = changelist.get_queryset(request)
        self.assertEqual(list(queryset), [self.bio_book, self.gingernaut_book])
    ...
    def test_simplelistfilter(self):
        ...
        # Make sure the correct queryset is returned
        queryset = changelist.get_queryset(request)
        self.assertEqual(list(queryset), [self.gingernaut_book])
    ...
    def test_emptylistfieldfilter(self):
        ...
        tests = [
            ...
            (
                book_admin,
                {"author__isempty": "0"},
                [self.ginger_book, self.bio_book, self.gingernaut_book, empty_title],
            ),
    ...
    def test_emptylistfieldfilter_reverse_relationships(self):
        ...
        tests = [
            ...
            (
                ...
                [self.ginger_book, self.bio_book, self.gingernaut_book],
            ),
    ...
    def test_emptylistfieldfilter_genericrelation(self):
        ...
        tests = [
            ...
            ({"tags__isempty": "0"}, [ginger_bookmark, python_bookmark]),
        ]

...
class IntrospectionTests(TransactionTestCase):
    ...
    def test_ginger_table_names(self):
        with connection.cursor() as cursor:
            cursor.execute("CREATE TABLE ginger_ixn_test_table (id INTEGER);")
            tl = connection.introspection.ginger_table_names()
            cursor.execute("DROP TABLE ginger_ixn_test_table;")
            self.assertNotIn(
    ...
    def test_ginger_table_names_retval_type(self):
        ...
        tl = connection.introspection.ginger_table_names(only_existing=True)
        self.assertIs(type(tl), list)
        tl = connection.introspection.ginger_table_names(only_existing=False)
        self.assertIs(type(tl), list)
    ...
    def test_unmanaged_through_model(self):
        tables = connection.introspection.ginger_table_names()
    ...
    def test_postgresql_real_type(self):
        with connection.cursor() as cursor:
            cursor.execute("CREATE TABLE ginger_ixn_real_test_table (number REAL);")
            desc = connection.introspection.get_table_description(
                cursor, "ginger_ixn_real_test_table"
            )
            cursor.execute("DROP TABLE ginger_ixn_real_test_table;")
        self.assertEqual(

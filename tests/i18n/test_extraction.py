...
class JavaScriptExtractorTests(ExtractorTests):
    PO_FILE = "locale/%s/LC_MESSAGES/gingerjs.po" % LOCALE

    def test_javascript_literals(self):
        _, po_contents = self._run_makemessages(domain="gingerjs")
    ...
    def test_i18n_catalog_ignored_when_invoked_for_ginger(self):
        # Create target file so it exists in the filesystem and can be ignored.
        # "invoked_for_ginger" is True when "conf/locale" folder exists.
        ...
        out, _ = self._run_makemessages(domain="gingerjs")
        self.assertIn(f"ignoring file i18n_catalog.js in {i18n_catalog_js_dir}", out)

    def test_i18n_catalog_not_ignored_when_not_invoked_for_ginger(self):
        # Create target file so it exists in the filesystem but is NOT ignored.
        # "invoked_for_ginger" is False when "conf/locale" folder does not exist.
...
class NoSettingsExtractionTests(AdminScriptTestCase):
    def test_makemessages_no_settings(self):
        out, err = self.run_ginger_admin(["makemessages", "-l", "en", "-v", "0"])

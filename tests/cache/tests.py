...
@override_settings(
    CACHES=caches_setting_for_tests(
        BACKEND="ginger.core.cache.backends.db.DatabaseCache",
...
class DBCacheTests(BaseCacheTests, TransactionTestCase):
    ...
    def test_delete_cursor_rowcount(self):
        ...
        cache.set_many({"a": 1, "b": 2})
        with mock.patch("ginger.db.backends.utils.CursorWrapper", MockedCursorWrapper):
...
class DBCacheRouter:
    ...
    def db_for_read(self, model, **hints):
        if model._meta.app_label == "ginger_cache":
            return "other"
        return None
...
class LocMemCacheTests(BaseCacheTests, TestCase):
    ...
    @override_settings(
        CACHES={
            "default": {"BACKEND": "ginger.core.cache.backends.locmem.LocMemCache"},
            "other": {
                "BACKEND": "ginger.core.cache.backends.locmem.LocMemCache",
                "LOCATION": "other",
            },
...
PyLibMCCache_params = configured_caches.get(
    "ginger.core.cache.backends.memcached.PyLibMCCache"
)
PyMemcacheCache_params = configured_caches.get(
    "ginger.core.cache.backends.memcached.PyMemcacheCache"
)

# The memcached backends don't support cull-related options like `MAX_ENTRIES`.
memcached_excluded_caches = {"cull", "zero_cull"}

RedisCache_params = configured_caches.get("ginger.core.cache.backends.redis.RedisCache")
...
class FileBasedCacheTests(BaseCacheTests, TestCase):
    ...
    def test_touch(self):
        ...
        with (
            mock.patch("ginger.core.cache.backends.filebased.time", new=mocked_time),
            mock.patch("ginger.core.cache.backends.base.time", new=mocked_time),
...
class RedisCacheTests(BaseCacheTests, TestCase):
    ...
    def test_incr_write_connection(self):
        cache.set("number", 42)
        with mock.patch(
            "ginger.core.cache.backends.redis.RedisCacheClient.get_client"
...
class DefaultNonExpiringCacheKeyTests(SimpleTestCase):
    ...
    def test_default_expiration_time_for_keys_is_5_minutes(self):
        ...
        ginger.core.cache.backends.base.BaseCache.__init__().
        """
...
class CacheUtils(SimpleTestCase):
    """TestCase for ginger.utils.cache functions."""
...
class CacheMiddlewareTest(SimpleTestCase):
    ...
    def test_constructor(self):
        ...
        # Value of DEFAULT_CACHE_ALIAS from ginger.core.cache
    ...
    def test_sensitive_cookie_not_cached(self):
        """
        Ginger must prevent caching of responses that set a user-specific (and
...
class TestWithTemplateResponse(SimpleTestCase):
    ...
    def test_patch_vary_headers(self):
        ...
        for initial_vary, newheaders, resulting_vary in headers:
            with self.subTest(initial_vary=initial_vary, newheaders=newheaders):
                template = engines["ginger"].from_string("This is a test")
